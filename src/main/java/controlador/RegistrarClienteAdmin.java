...
public class RegistrarClienteAdmin extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (clienteDao.registrar(cliente)) {
            ...
            request.getRequestDispatcher("clientes.jsp").forward(request, response); // Redirige a la página de login
        } else {
            request.setAttribute("mensaje", "Error al registrar cliente"); // Mensaje de error
            request.getRequestDispatcher("RegistroClienteAd.jsp").forward(request, response); // Redirige a la página de error
        }
    }
}
